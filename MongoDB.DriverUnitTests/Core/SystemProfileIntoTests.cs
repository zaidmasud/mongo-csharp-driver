...
namespace MongoDB.DriverUnitTests
{
    ...
    public class SystemProfileInfoTests
    {
        ...
        {
            var info = new SystemProfileInfo(SerializationConfig.Default)
            {
        ...
        public void TestAll()
        {
            var info = new SystemProfileInfo(SerializationConfig.Default)
            {
                ...
                Info = "info",
                KeyUpdates = 4,
                LockStatistics = new SystemProfileLockStatistics(SerializationConfig.Default)
                {
                    TimeAcquiring = new SystemProfileReadWriteLockStatistics(SerializationConfig.Default)
                    {
                        Read = TimeSpan.FromMilliseconds(10),
                        Write = TimeSpan.FromMilliseconds(20)
                    },
                    TimeLocked = new SystemProfileReadWriteLockStatistics(SerializationConfig.Default)
                    {
